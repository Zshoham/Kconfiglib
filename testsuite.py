...
"""[1:])
    ...
    # CMake output matches the .config symbol set and preserves strings
    # verbatim.

    c = Kconfig("Kconfiglib/tests/Kcmake")
    c.syms["TEXT"].set_value(r'a;${HOME}]]]=]\q')
    verify("saved" in c.write_cmake_config(config_test_file),
           "initial CMake configuration write was reported as unchanged")
    verify_file_contents(config_test_file, """\
# Generated by Kconfiglib. Do not edit.
set(CONFIG_MODULES y)
set(CONFIG_ENABLED y)
set(CONFIG_DISABLED n)
set(CONFIG_MODULE m)
set(CONFIG_NUMBER 42)
set(CONFIG_ADDRESS 0x1000)
set(CONFIG_TEXT [==[a;${HOME}]]]=]\\q]==])
set(KCONFIG_SYMBOLS "CONFIG_MODULES;CONFIG_ENABLED;CONFIG_DISABLED;\
CONFIG_MODULE;CONFIG_NUMBER;CONFIG_ADDRESS;CONFIG_TEXT")
""")
    verify_cmake_config(c, config_test_file)
    verify("No change" in c.write_cmake_config(config_test_file),
           "unchanged CMake configuration was rewritten")

    # String values that exercise bracket-argument edge cases. Each must
    # round-trip through the parser in verify_cmake_config(), which reads
    # bracket arguments the same way CMake does.
    for text in (
        "",                   # Empty string
        "a]",                 # Ends with a prefix of the level-0 terminator
        "x]=",                # Ends with ']=' at level 0
        "]]x]=",              # Needs level 2: ']]' inside, ends with ']='
        "]]==]",              # Terminator-like runs for several levels
        "\nleading newline",  # CMake drops a newline after the bracket
        'quotes " \\ ${VAR} ; ) #',
    ):
...
def test_sanity(arch, srcarch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        verify_cmake_config(kconf, os.path.join(tmpdir, "config.cmake"))
...
def test_defconfig(arch, srcarch):
    ...
    for defconfig in defconfigs:
        ...
        verify_cmake_config(kconf, "._config.cmake")
...
def verify_cmake_config(kconf, filename):
    # Generate and parse the CMake output, then compare it to the symbols
    # that should be written out. This is called for each architecture and
    # defconfig, including defconfigs found in subdirectories by
    # defconfig_files().

    kconf.write_cmake_config(filename)
    with open(filename, "r", encoding="utf-8") as f:
        contents = f.read()

    header = "# Generated by Kconfiglib. Do not edit.\n"
    verify(contents.startswith(header),
           "CMake configuration has a missing or invalid header")

    # The same symbol set and value transformations as in
    # _cmake_config_contents()
    expected = []
    for sym in kconf.unique_defined_syms:
        ...
        if sym.orig_type is HEX and not val.startswith(("0x", "0X")):
            val = "0x" + val
        expected.append((kconf.config_prefix + sym.name, val))

    trailer = 'set(KCONFIG_SYMBOLS "{}")\n' \
              .format(";".join(name for name, _ in expected))
    verify(contents.endswith(trailer),
           "CMake configuration is missing or has a wrong KCONFIG_SYMBOLS "
           "list")

    # STRING values are bracket arguments. The other emitted values cannot
    # contain ')' or newlines and are plain arguments (or "" when empty).
    set_re = re.compile(
        r"set\(([^ )]+) (?:\[(=*)\[(.*?)\]\2\]|([^\n)]*))\)\n", re.DOTALL)
    parsed = []
    pos = len(header)
    end = len(contents) - len(trailer)
    for match in set_re.finditer(contents, pos, end):
        verify(match.start() == pos,
               "invalid CMake configuration near {!r}"
               .format(contents[pos:match.start()]))
        if match.group(3) is not None:
            # Bracket argument. Like CMake, drop a newline immediately
            # following the opening bracket.
            val = match.group(3)
            if val.startswith("\n"):
                val = val[1:]
        else:
            # set(X "") defines X as empty, while set(X ) would unset it
            val = "" if match.group(4) == '""' else match.group(4)
        parsed.append((match.group(1), val))
        pos = match.end()

    verify(pos == end,
           "invalid trailing CMake configuration text {!r}"
           .format(contents[pos:end]))
...
def test_cmake_module():
    # Exercise the includeable CMake wrapper in a small standalone project.
    if shutil.which("cmake") is None:
        print("Skipping CMake module test (cmake was not found).")
        return
